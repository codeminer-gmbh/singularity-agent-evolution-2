...
def run_improvement(settings: AgentSettings) -> RunReport:
    ...
    changed = workspace.digest() != started_from
...
_VERIFICATION_RECEIPT = "memories/verification-receipt.md"
_RECEIPT_RESULT_LIMIT = 4_000


def _write_verification_receipt(workspace: Workspace, session: ToolAgentSession) -> None:
    """Record exactly which commands were available as improvement evidence.

    The receipt is generated by the runner rather than trusted to a final
    summary.  It deliberately records failed commands too: a later iteration
    must not mistake a command invocation for a passing check.
    """
    lines = [
        "# Verification receipt",
        "",
        "This file is generated after a source-changing improvement run. It records "
        "the `run_command` observations made by that run; it does not establish "
        "that a command tested the claimed change.",
        "",
    ]
    if not session.verification_commands:
        lines.append("No `run_command` verification was run.")
    for record in session.verification_commands:
        status = "tool error" if record.is_error else "observed"
        rendered = " ".join(json.dumps(part) for part in record.command)
        result = record.result[:_RECEIPT_RESULT_LIMIT]
        if len(record.result) > _RECEIPT_RESULT_LIMIT:
            result += "\n[receipt truncated]"
        lines.extend(
            [
                f"## Step {record.step}: {status}",
                "",
                f"Command: `{rendered}`",
                "",
                "```text",
    ...
    workspace.write_text(_VERIFICATION_RECEIPT, "\n".join(lines))
