...
def query_sqlite(path: Path, query: str, *, max_rows: int = 200) -> str:
    ...
    if path.stat().st_size > MAX_DATABASE_BYTES:
        raise DatabaseError(
            f"{path.name!r} exceeds the {MAX_DATABASE_BYTES}-byte database limit."
        )
    if not isinstance(query, str) or not query.strip():
        raise DatabaseError("query must be a non-empty SQL string.")
    if not isinstance(max_rows, int) or isinstance(max_rows, bool) or not 1 <= max_rows <= MAX_QUERY_ROWS:
...
def _read_only_authorizer(action: int, arg1: str | None, arg2: str | None, database: str | None, trigger: str | None) -> int:
